...
```{r, fig.cap="Figure1", include=TRUE}
data<-prep_data_in%>%
...
```{r, include=TRUE, fig.cap="Correlation Table"}
...
```{r Synthetic control construct, echo=FALSE}
dataprep_out <- dataprep(
  foo = prep_data_in,
  predictors = c("poverty", "income", "bmprison", "alcohol", "aidscapita", "black", "perc1519"),
  predictors.op = "mean",
...
```{r, include=TRUE, fig.cap="Treated (Texas) Vs Synthetic Groups"}
...
```{r, fig.cap="The Relative Importance of Each Unit", include=TRUE}
...
```{r, include=TRUE, cache=TRUE, fig.cap="Actual Difference Between Teen Pregnancy rate After and Before Texas Prison Expansion"}
